...
| 2026-09-21 | 02 tin đăng | **Demo 10 xe** (`supabase/seed/demo-10-xe.sql`, gỡ bằng `xoa-demo-10-xe.sql` — xoá mềm): gắn nhãn `[DEMO]`, chủ xe `demo@thuexenhanh.invalid` không đăng nhập được, SĐT `0000000000`, **không** ảnh/đánh giá/lượt xem/tích xanh/biển số. 9 `dang_hien_thi` + 1 `sap_het_han`. Thử trên Postgres nhúng 20/20 đạt (chạy lại không nhân đôi, anon thấy 10 thẻ, gỡ xong còn 0). **Chưa nạp vào Supabase thật — anh dán vào SQL Editor. Gỡ trước khi mở cho người dùng thật** | `supabase/seed/*` |
